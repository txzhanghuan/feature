...
## 更新摘要
**变更内容**
- 新增异常处理章节，详细介绍CalculateException和FeatureCreationException两种异常类型的使用场景和错误处理策略
- 增强了FeatureContext的依赖解析和错误传播机制，完善了根因定位和异常链传播
...
- 异常处理机制（CalculateException、FeatureCreationException）
- 循环依赖检测与规避策略
- 缓存系统实现与使用建议
...
- 异常处理：CalculateException、FeatureCreationException
...
- [CalculateException.java:1-73](file://src/main/java/com/github/zh/engine/exception/CalculateException.java#L1-L73)
- [FeatureCreationException.java:1-62](file://src/main/java/com/github/zh/engine/exception/FeatureCreationException.java#L1-L62)
...
- 异常处理 CalculateException/FeatureCreationException：提供计算失败和特征创建失败的异常处理机制。
...
#### CalculateException - 计算异常
CalculateException 是引擎抛出的主要运行时异常，用于处理特征计算过程中的各种失败情况：

**异常类型与使用场景**：
- 计算超时：当计算超过配置的超时时间时抛出
- 无可用特征：当请求的特征集合为空时抛出
- 缺少必需特征或输入参数：当依赖的特征或输入参数缺失时抛出
- 单个特征计算失败：当某个特征执行过程中发生异常时抛出

**异常特性**：
- 继承自 RuntimeException，无需显式声明或捕获
- 支持多种构造函数，包括消息、原因和组合形式
- 保持异常链完整性，便于根因分析
...
// 缺少特征异常
throw new CalculateException("缺少Feature或者输入参数: " + featureName);

// 根因异常
throw new CalculateException("Calculate failed at feature: " + errorFeature, rootCause);
```

#### FeatureCreationException - 特征创建异常
FeatureCreationException 是在应用启动期间特征Bean创建失败时抛出的异常：

**异常类型与使用场景**：
- 动态类生成失败：Javassist生成类时出现错误
- 特征方法签名无效：方法参数或返回类型不符合要求
- Bean实例化失败：反射创建实例时发生异常

**异常特性**：
- 继承自 BeansException，与Spring的Bean创建错误处理机制集成
- 支持嵌套异常信息，便于追踪底层原因
- 在NativeFeatureProcessor中统一处理和抛出

**使用示例**：
```java
// 动态类生成失败
throw new FeatureCreationException("Failed to create feature bean", cause);

// 方法签名无效
throw new FeatureCreationException("Invalid method signature: " + methodName);
```
...
```mermaid
flowchart TD
Start(["开始计算"]) --> CheckParams["检查参数就绪"]
CheckParams --> ParamsReady{"参数是否就绪？"}
ParamsReady --> |否| Wait["等待父节点完成"]
ParamsReady --> |是| CheckParentError["检查父节点错误"]
CheckParentError --> ParentError{"父节点是否有错误？"}
ParentError --> |是| SetError["设置错误父节点"]
ParentError --> |否| Execute["执行特征计算"]
Execute --> ExecSuccess{"执行是否成功？"}
ExecSuccess --> |是| Success["标记SUCCESS"]
ExecSuccess --> |否| SetFailed["标记FAILED并设置异常"]
SetFailed --> FastFail["设置快速失败标志"]
SetError --> FastFail
FastFail --> Propagate["传播到子节点"]
Success --> Propagate
Propagate --> End(["结束"])
```
...
- [FeatureContext.java:348-358](file://src/main/java/com/github/zh/engine/co/FeatureContext.java#L348-L358)
...
- [FeatureEntity.java:106-181](file://src/main/java/com/github/zh/engine/co/FeatureEntity.java#L106-L181)
- [FeatureContext.java:348-358](file://src/main/java/com/github/zh/engine/co/FeatureContext.java#L348-L358)
- [NativeFeatureProcessor.java:112-116](file://src/main/java/com/github/zh/engine/processor/NativeFeatureProcessor.java#L112-L116)
...
### CalculateException - 计算异常处理

#### 异常类型与触发场景
CalculateException 是引擎在计算过程中抛出的主要异常，涵盖以下场景：
...
private void checkFail() {
    if (fastFail) {
        FeatureEntity featureEntity = featureEntitiesPool.values().stream()
            .filter(it -> it.getStatus().get().equals(FeatureStates.FAILED) && it.getError() != null)
            .findFirst().get();
...
#### 异常处理策略
**1. 快速失败机制**
- 一旦检测到任何特征失败，立即设置 fastFail 标志
- 阻止剩余特征的执行
- 通过 CountDownLatch 同步等待所有任务完成
...
### FeatureCreationException - 特征创建异常处理

#### 异常触发场景
FeatureCreationException 主要在应用启动期间的特征Bean创建阶段抛出：
...
#### Spring集成特性
FeatureCreationException 继承自 BeansException，与Spring框架深度集成：
...
```java
try {
    result = featureEngine.calc(inputData, features, timeout);
} catch (CalculateException e) {
    if (e.getMessage().contains("计算超时")) {
        // 增加超时时间或优化慢特征
        handleTimeout(e);
    }
}
...
```java
try {
    result = featureEngine.calc(inputData, features);
} catch (CalculateException e) {
    String rootCause = e.getMessage().split(": ")[2];
    log.error("根因特征: {}", rootCause);
    // 针对特定特征进行修复
}
```
...
try {
    ...
    log.error("特征创建失败: {}", e.getMessage());
    // 检查方法签名和依赖
    validateFeatureDefinition(e);
}
```
...
本引擎通过 DAG 编排与并行执行，有效消除了重复调用与串行瓶颈。在高级用法中，应重点关注异常处理机制的正确使用、环依赖的规避、缓存策略的选择、复杂场景的拆分与组合，以及性能参数的合理调优。通过完善的异常处理和根因定位机制，可快速定位并解决问题。

新增的CalculateException和FeatureCreationException异常类型为引擎提供了更精细的错误处理能力，配合增强的FeatureContext依赖解析和错误传播机制，使得异常处理更加准确和高效。当前版本虽然提供了环检测工具但未启用自动阻止机制，需要开发者在编码阶段严格遵守无环依赖的原则。
...
**异常处理测试**：
- CalculateException 各种构造函数测试
- FeatureCreationException Spring集成测试
- 异常链保持和根因定位测试
