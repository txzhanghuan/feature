...
## 更新摘要
**变更内容**
- 新增功能扩展架构章节，详细介绍自定义Bean开发、外部Bean集成和后置处理器
- 更新高级配置选项，新增外部Bean集成和后置处理器相关配置
- 新增复杂特征计算场景，包含动态特征和运行时扩展
- 更新故障排查指南，新增外部Bean和后置处理器相关问题诊断
...
- 自定义计算Bean：通过继承 AbstractFeatureBean 实现复杂的业务逻辑
- 外部Bean集成：通过 calcWithOuterFeatureBean 方法动态注入外部Bean
- 后置处理器：通过 FeatureBeanPostProcessor 接口在Bean初始化后进行拦截处理
...
### 自定义计算Bean开发
通过继承 AbstractFeatureBean 来创建自定义计算Bean，支持灵活的参数传递和结果处理。
...
**章节来源**
- [README.md:241-281](file://README.md#L241-L281)
- [AbstractFeatureBean.java:1-28](file://src/main/java/com/github/zh/engine/co/AbstractFeatureBean.java#L1-L28)
- [OuterFeatureBean.java:1-16](file://src/test/java/com/github/zh/bean/OuterFeatureBean.java#L1-L16)

### 外部Bean集成
通过 calcWithOuterFeatureBean 方法将外部自定义Bean注入到计算上下文中，实现动态扩展和运行时组合。
...
#### 外部Bean集成的优势
- 运行时动态扩展：无需重启即可添加新的计算逻辑
- 灵活的参数传递：支持复杂的参数组合和条件判断
...
- [FeatureContext.java:148-178](file://src/main/java/com/github/zh/engine/co/FeatureContext.java#L148-L178)

### FeatureBean后置处理器
实现 FeatureBeanPostProcessor 接口，可在Bean初始化后进行自定义处理，类似于Spring的BeanPostProcessor。
...
#### 后置处理器的工作流程
1. NativeFeatureProcessor 创建NativeFeatureBean实例
2. 调用 doFeatureBeanPostProcessor 方法
3. 遍历所有FeatureBeanPostProcessor实例
4. 根据returnClass匹配目标Bean类型
5. 应用后置处理器逻辑
6. 返回处理后的Bean

**章节来源**
- [README.md:306-319](file://README.md#L306-L319)
- [FeatureBeanPostProcessor.java:1-21](file://src/main/java/com/github/zh/engine/interfaces/FeatureBeanPostProcessor.java#L1-L21)
...
- [NativeFeatureProcessor.java:53-54](file://src/main/java/com/github/zh/engine/processor/NativeFeatureProcessor.java#L53-L54)
...
- 组件耦合：
  ...
  - AbstractFeatureProcessor 依赖 FeatureBeanPostProcessor 列表。
- 外部依赖：
...
Engine --> AFP["AbstractFeatureProcessor"]
AFP --> FBPP["FeatureBeanPostProcessor"]
FBPP --> NFB["NativeFeatureBean"]
```
...
- [FeatureBeanPostProcessor.java:3](file://src/main/java/com/github/zh/engine/interfaces/FeatureBeanPostProcessor.java#L3)
...
- 外部Bean性能优化
  - 外部Bean应避免长时间阻塞操作，建议使用异步处理或缓存机制。
  - 合理设置外部Bean的依赖关系，避免过深的依赖层次。
...
- 外部Bean问题排查
  - 外部Bean未执行：检查Bean的output属性和依赖关系配置。
  - 外部Bean参数错误：验证传入的参数类型和数量是否正确。
  - 外部Bean性能问题：检查外部Bean的执行时间和资源消耗。
- 后置处理器问题排查
  - 后置处理器未生效：确认Bean类型匹配和returnClass配置正确。
  - 后置处理器异常：检查后置处理器中的异常处理逻辑。
...
## 结论
本引擎通过 DAG 编排与并行执行，有效消除了重复调用与串行瓶颈。在高级用法中，应重点关注环依赖的规避、缓存策略的选择、复杂场景的拆分与组合，以及性能参数的合理调优。通过日志与根因定位机制，可快速定位并解决问题。新增的功能扩展架构进一步增强了引擎的灵活性和可扩展性，支持自定义Bean开发、外部Bean集成和后置处理器等高级特性。
...
- 扩展点
  ...
  - 处理器注册：通过 Spring 容器自动发现和注册后置处理器
